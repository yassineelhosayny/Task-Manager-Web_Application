...
function renderTasks(session, task) {
  const li = document.createElement("li");
  const div = document.createElement("div");
  const div2 = document.createElement("div");
  const input = document.createElement("input");
  const label = document.createElement("label");
  const spanNomeUtente = document.createElement("span");
  ...
  spanNomeUtente.innerText = `Task di: ${task.nome_proprietario}`;
  spanNomeUtente.classList.add("spanNomeUtente");
  ...
  if (session.id === task.id_utente) {
    if (task.completato === 1) input.checked = true;
  ...
  div2.appendChild(span);
    //nome Proprietario task
  if(spanNomeUtente){
    div2.appendChild(spanNomeUtente);
    }
  ...
  if (session.id === task.id_utente || session.ruolo === "admin") {
  ...
  li.dataset.id = task.id;
      console.log("task added to dom", li.dataset.id);
...
async function fetchTasks(filterOBJ = {}) {
  try {
    ...
    if(!tasks.success && tasks.redirectTo){
      window.location.href = tasks.redirectTo;
      return;
    }
...
formaddingTask.addEventListener("submit", async (e) => {
  ...
  try {
    if (!editON) {
      ...
      if (res.ok || data.success) filterSelected("tutti");
      ...
      if (res.ok || update.success) filterSelected("tutti");
...
taskResult.addEventListener("click", async (e) => {
  ...
  try {
    ...
    if (res.ok) {
      ...
      formaddingTask.progetto.value =
        task.progetto === "personale" ? "Personale" : "upo";
...
taskResult.addEventListener("change", async (e) => {
  ...
  try {
    ...
    const data = await res.json();
    if (!res.ok) console.log("Errore nella richiesta dal server!.");
  ...
  if (descrizione) {
    try {
      const res = await fetch(
        `/task/cercaTask?descrizione=${encodeURIComponent(descrizione)}`
      );
      const tasks = await res.json();
      ...
      if (tasks.success && tasks.data.length >= 1) {
        ...
        document.getElementById("TaskResult").innerHTML =
          "<li class='list-group-item'>Nessun task trovato</li>";
      }
